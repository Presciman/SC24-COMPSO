...
class KFAC(optim.Optimizer):
    ...
    def step(self, closure=None):
        ...
        if self.comm_method in [CommMethod.MEM_OPT, CommMethod.HYBRID_OPT]:
            #print('broadcast gradients')
            self.broadcast_gradients()
            #print('getting out of broadcast gradients')#Baixi
            #Baixi
            #self.decompose_gradients()#Baixi decompose after broadcast
            #print('decompressed')
    ...
    #Baixi decompose gradients
    #@trace(sync=True)
    def decompose_gradients(self):
        '''Same input, decompress and same output as the broadcast_gradients()'''
        handles = []
        for layer in self.layers:
            handles.extend(layer.decompose_gradient())
