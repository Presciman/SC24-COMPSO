...
class KFACLayer(object):
    def __init__(self,
        ...
        os.environ['NGRAD'] = str(0)
        self.error_bound = float(os.environ['ERROR_BOUND'])
        #self.grad_shape = None#Baixi
        #self.grad_dtype = None#Baixi
        lib_extention = "so" if platform.system() == 'Linux' else "dylib"
        self.sz = SZ("/ccs/home/dtao/baixi/SZ3/build/tools/sz3c/libSZ3c.{}".format(lib_extention))
        #print('BX initialized!')#Baixi
    ...
    def broadcast_gradient(self):
        ...
        '''
        #Baixi
        dimm = self.preconditioned_gradient[0].size(dim=1)
        CR = 1
        if dimm > 512:
            self.preconditioned_gradient[0] = self.preconditioned_gradient[0][:,:int(dimm//CR)].contiguous()
            #print(self.preconditioned_gradient[0].size())
        
        
        #print('Ranks:{},Rank:{},Group:{}'.format(self.compute_grad_ranks,comm.backend.rank(),group))
        #print('gradient broadcast!')#Baixi
        print('Rank:{},Tensor Size:{}'.format(comm.backend.rank(),self.preconditioned_gradient[0].size()))
        '''
...
#        arr_tmp = self.preconditioned_gradient[0].detach().cpu().clone().numpy().astype(np.single)
#        if np.count_nonzero(arr_tmp) > 512 and comm.backend.rank() == 0:
#            number = os.environ['NGRAD']
#            fname = '/gpfs/alpine/csc337/scratch/dtao/baixi/error_impact/resnet/inv_tsr_'+str(number)+'.npy'
#            np.save(fname,arr_tmp)
#            os.environ['NGRAD'] = str(int(number)+1)
        
        #if comm.backend.rank() == 0:
            #print(self.preconditioned_gradient)
        #Baixi compress gradients
        #If has more than 512 zeros
        self.grad_shape=None
        self.grad_dtype=None
        #print('before compress!')#Baixi
        arr_tmp = self.preconditioned_gradient[0].detach().cpu().clone().numpy().astype(np.single)
        if np.count_nonzero(arr_tmp) > 512:
            #self.grad_shape.append(arr_tmp.shape)
            #self.grad_dtype.append(arr_tmp.dtype)
            self.grad_shape=arr_tmp.shape
            self.grad_dtype=np.single
            #compress
            #if comm.backend.rank() == 0:
                #print(self.preconditioned_gradient[0].dtype)
            data_cmpr, cmpr_ratio = self.sz.compress(arr_tmp, 0, self.error_bound*(arr_tmp.max()-arr_tmp.min()), 0, 0)
            #if 0 == comm.backend.rank():
                #print(data_cmpr.dtype)
                #print('compression ratio: {}'.format(cmpr_ratio),flush=True)
            data_dec = self.sz.decompress(data_cmpr, arr_tmp.shape, np.single)
            
#            if int(os.environ['EPOCHS']) < 2:
#                number = os.environ['NGRAD']
#                fname = '/gpfs/alpine/csc337/scratch/dtao/baixi/error_impact/resnet/'+os.environ['ERROR_BOUND']+'/inv_tsr_'+str(number)+'.npy'
#                np.save(fname,arr_tmp)
#                os.environ['NGRAD'] = str(int(number)+1)
            
            self.preconditioned_gradient[0] = torch.from_numpy(data_dec.astype(np.single)).detach().clone().cuda()
            #if comm.backend.rank() == 0:
                #print(self.preconditioned_gradient[0].dtype)
            #self.preconditioned_gradient[0] = torch.from_numpy(data_cmpr).cuda()
            #del arr_tmp, data_cmpr, data_dec
    ...
    #Baixi decompose here
    def decompose_gradient(self):
        #Baixi decompose
        print('before decompose!',flush=True)
        data_cmpr = self.preconditioned_gradient[0].detach().cpu().numpy()
        print(data_cmpr.dtype)
        print(data_cmpr.shape)
        data_dec = self.sz.decompress(data_cmpr, ori_shape, ori_dtype)
        self.preconditioned_gradient[0] = torch.from_numpy(data_dec).detach().clone().cuda()
        print('after decompose!',flush=True)
